...
# Tab 1: Mobility calculator (now standalone, with Save/Load/Examples)
...
def _project_root():
    # Program plan: default file dialogs should start in the folder that contains Main_Gui.py.
    # That is one level up from this tab's directory.
...
class ScrollFrame(ttk.Frame):
    ...
    def _on_wheel(self, e):
        delta = -e.delta if hasattr(e, "delta") and e.delta else ( -120 if getattr(e, "num", 0)==4 else 120 )
        self.canvas.yview_scroll(int(delta/120), "units")

class MobilityTab(ttk.Frame):
    """Implements the Mobility calculator tab with Save/Load/Examples."""
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)

        # Styles (safe if defined multiple times)
        ...
        # Save/Load/Examples row (Program Plan requirement)
        bar = ttk.Frame(root)
        ...
        # Examples buttons (quick inserts)
        ...
        ttk.Button(opt_frame, text="Output Fancy (LaTeX/MathType)", command=self._output_fancy).pack(side="left")
        ...
        # Build Examples list from files in this tab's directory
    ...
    def _build_joint_table_header(self):
        ...
        for c, t in enumerate(hdrs):
            ttk.Label(self.joint_rows_frame, text=t, style="Header.TLabel").grid(row=r, column=c, padx=4, pady=(0,4), sticky="w")
    ...
    def _rebuild_joint_rows(self):
        ...
        for i in range(j):
            r = i + 1
            lab = ttk.Label(self.joint_rows_frame, text=str(i+1))
            ...
            cb = ttk.Combobox(self.joint_rows_frame, textvariable=typ_var,
                              values=("R","P","C","Custom"), width=8, state="readonly")
            ...
            def _on_sel(_e=None, tv=typ_var, fv=fi_var, e=fi_entry):
                t = tv.get()
                if t == "R" or t == "P":
                    fv.set(1.0); e.configure(state="normal")
                elif t == "C":
                    fv.set(2.0); e.configure(state="normal")
                else:
            ...
            widgets = (lab, cb, fi_entry, a_entry, b_entry)
    ...
    def _recompute(self):
        ...
        sum_fi, fi_list = self._sum_fi(self.joint_rows)

        if spherical:
            if n == j:
                M_primary = sum_fi - 3 + deltaM
                primary_name, primary_eq_sym, primary_eq_num = "Spherical (single-chain closed loop)", "M = Σfi − 3", f"M = {sum_fi:.3g} − 3"
            else:
                M_primary = 3*(n-1) - sum((3-fi) for fi in fi_list) + deltaM
                primary_name, primary_eq_sym, primary_eq_num = "Spherical (general)", "M = 3(n − 1) − Σ(3 − fi)", f"M = 3({n} − 1) − Σ(3 − fi)"
        else:
            if n == j:
                M_primary = sum_fi - 6 + deltaM
                primary_name, primary_eq_sym, primary_eq_num = "Spatial (single-chain closed loop)", "M = Σfi − 6", f"M = {sum_fi:.3g} − 6"
            else:
                M_primary = 6*(n-1) - sum((6-fi) for fi in fi_list) + deltaM
                primary_name, primary_eq_sym, primary_eq_num = "Spatial (general)", "M = 6(n − 1) − Σ(6 − fi)", f"M = 6({n} − 1) − Σ(6 − fi)"

        M_spatial_general  = 6*(n-1) - sum((6-fi) for fi in fi_list) + deltaM
        M_spatial_closed   = (sum_fi - 6 + deltaM) if n==j else None
        M_spherical_general= 3*(n-1) - sum((3-fi) for fi in fi_list) + deltaM
        M_spherical_closed = (sum_fi - 3 + deltaM) if n==j else None
        group_num          = (sum_fi - 3) if n==j else None

        self.output_text.configure(state="normal")
        self.output_text.delete("1.0","end")
        self.output_text.insert("end", f"Primary result ({primary_name}):\n  {primary_eq_sym}\n")
        if show_steps:
            if "Σ" in primary_eq_sym:
                if spherical and (n != j):
                    sigma_terms = " + ".join([f"(3 − {fi:g})" for fi in fi_list]) or "0"
                    self.output_text.insert("end", f"  {primary_eq_num} = 3*({n-1}) − ({sigma_terms})\n")
                elif (not spherical) and (n != j):
        ...
        if n==j: self.output_text.insert("end", f"  Spatial closed:         M = Σfi − 6          ⇒ {M_spatial_closed:.6g}\n")
        self.output_text.insert("end", f"  Spherical general:      M = 3(n−1) − Σ(3−fi)  ⇒ {M_spherical_general:.6g}\n")
        if n==j: self.output_text.insert("end", f"  Spherical closed:       M = Σfi − 3          ⇒ {M_spherical_closed:.6g}\n")
        ...
        if show_verbose:
            ...
            if spherical:
                self.output_text.insert("end","  For spherical mechanisms each link has 3 DOF: M = 3(n−1) − Σ(3−fi). For a single-chain loop (n=j), M = Σfi − 3.\n")
            else:
                self.output_text.insert("end","  For spatial mechanisms each link has 6 DOF: M = 6(n−1) − Σ(6−fi). For a single-chain loop (n=j), M = Σfi − 6. The equivalent spherical mobility (Group) is G = Σfi − 3.\n")
            self.output_text.insert("end","  ΔM accounts for special geometry (axis alignments, etc.).\n")

        self.output_text.insert("end", "\nInputs summary:\n")
        self.output_text.insert("end", f"  n = {n}, j = {j}, spherical={bool(self.is_spherical_var.get())}, ΔM = {deltaM}\n")
        self.output_text.insert("end", f"  fi list (per joint): {', '.join(f'{fi:g}' for fi in fi_list) if fi_list else '(none)'}\n")
    ...
    def _output_fancy(self):
        # Placeholder per Program Plan; easy to wire to MathJax/Latex export later.
        messagebox.showinfo("Output Fancy", "LaTeX/MathType export coming soon.")
    ...
    def _apply_state_from_dict(self, d):
        ...
        n   = int(d.get("n", 1))
        # j is recomputed from joint[] keys if present
        joint_idxs = sorted({int(k.split("[",1)[1].split("]",1)[0])
                             for k in d.keys() if k.startswith("joint[")})
        j = int(d.get("j", len(joint_idxs)))
        if len(joint_idxs) > 0:
            j = max(j, 1 + max(joint_idxs))
        ...
        for i in range(min(j, len(self.joint_rows))):
            ...
            t = d.get(f"joint[{i}].type", st["typ"].get()); st["typ"].set(t)
    ...
    def _refresh_examples_list(self):
        # Example files follow: Example_{name}_Mobility.csv in this tab's directory
